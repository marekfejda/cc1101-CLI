//
// CC1101 interactive terminal CLI tool
...
// based on SmartRC library by Little_S@tan
...
static void exec(char *cmdline)
{
  ...
  } else if (strcmp_P(command, PSTR("flusheeprom")) == 0) {
    for (setting = 0; setting < EPROMSIZE ; setting++)
    { // copying byte after byte from SRAM to EEPROM
      EEPROM.write(setting, 0);
    }
    Serial.print(F("\r\Flushing complete.\r\n\r\n"));
